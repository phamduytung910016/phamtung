        ...
        <div class="content">
            <div class="row">
                ...
                <div class="col-sm-4 col-4 text-right m-b-10">
                    <a href="{{ route('get_them_don_vi_bao_tri') }}" class="btn btn-rounded float-right"
                    ...
                    @if (isset($search))
                        @foreach ($search as $result)
                            <tr>
                                ...
                                <td>{{ $result->email }}</td>
                                <td>
                                    <a href="/admin/donvibaotri/get-edit/{{ $result->id }}"><i
                                            class="fa fa-pencil-square-o " style="font-size: 15px;" title="Chỉnh sửa"
                                            aria-hidden="true"></i></a>&nbsp;
                                    <a onclick="return confirm('Bạn có chắc chắn xóa?')"
                                        href="/admin/donvibaotri/delete/{{ $result->id }}"><i class="fa fa-trash"
                                            style="font-size: 15px;" title="Xóa" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
